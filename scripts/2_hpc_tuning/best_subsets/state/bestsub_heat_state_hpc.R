...
bestsub_predictors <- c("wave_in_months", "cc_heat_threshold_f", "cc_heat_wetbulb_threshold_f", 
                        "y_mean_impervious", "y_centroid_lat", "y_pct_presdem",
                        "cdc_svi_demo", "cdc_svi_housing", "cdc_svi_socioecon", 
                        "fema_HWAV_HLRP", "fema_HWAV_HLRB", "fema_HWAV_EVNTS", 
                        "fema_HWAV_EXPT", "fema_HWAV_EALB", "fema_HWAV_EALP")


all_combinations <- lapply(1:length(bestsub_predictors), function(x) combn(bestsub_predictors, x))
...
best_result <- bic_values[[which.min(sapply(bic_values, function(x) x$bic))]]
best_combination <- best_result$predictors
...
save(best_result, best_combination, best_bic, file=paste0(output_path, "bestsub_heat_county_tuning.rda"))


# TO DO: 
# 1. add cross-validation to this 
# 2. Evaluate models for AUC performance instead of BIC! 
