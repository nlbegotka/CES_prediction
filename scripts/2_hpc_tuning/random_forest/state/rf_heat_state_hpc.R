# Run random forest model for state_level heat worry in Grace cluster
...
# Read in data 
load(paste0(input_path, "poll_state_heat.rda")) # data

#==============================================================================#
# 1. Do final data cleaning -- random forest and model specific
#==============================================================================#
poll_state <- poll_state %>% 
  select(-c(wave_ordered, geoid))

#==============================================================================#
# 2. Run random forest model for state
#==============================================================================#
#-------------------------------------------------------------------------------
# 2.1. Split data into train and test
#-------------------------------------------------------------------------------
set.seed(42)
split_state <- initial_split(poll_state, prop = 0.8, strata = "geoid")
...
train_state_processed <- rf_recipe %>%
  prep() %>%
  bake(train_state)
...
rf_wflow_final <- last_fit(rf_wflow_best, split = split_state)
...
save(tuning_df, rf_tune_best, train_auc, test_auc, predictor_importance, metrics, 
     conf_mat, file=paste0(output_path, "rf_heat_state_tuning.rda"))
